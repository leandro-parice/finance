<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">Imagens</h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <a href="{{ route('image-file.create') }}" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded">Nova Imagem</a>

                    <table class="table-auto w-full mt-4 border-collapse border border-gray-600">
                        <thead>
                            <tr>
                                <th class="border border-gray-600">Arquivo</th>
                        ...
                        <tbody>
                            @foreach ($images as $key => $image)
                                <tr class="border border-gray-600 text-sm {{ $key % 2 == 0 ? 'bg-gray-800' : 'bg-gray-700' }}">
                                    <td class="border border-gray-600 text-center py-2 px-4">
                                        <a href="{{ asset('storage/image-files/' . $image->image) }}" target="_blank" class="text-blue-500 hover:underline">{{ $image->image }}</a>
                                    </td>
                                    <td class="border border-gray-600 text-center py-2 px-4">
                                        @if ($image->status == 'active')
                                            <span class="bg-green-500 text-white text-xs py-1 px-2 rounded">Processado</span>
                                        @else
                                            <span class="bg-yellow-700 text-white text-xs py-1 px-2 rounded">Pendente</span>
                                        @endif
                                    </td>
                                        ...
                                        <div id="modal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 " x-show="openModal">
                                            <div class="bg-gray-800 p-6 rounded-lg shadow-lg w-96">
                                                ...
                                                <div class="py-3 sm:flex sm:align-center sm:justify-center sm:gap-4">
                                                    <form x-ref="form" action="{{ route('image-file.destroy', $image->id) }}" method="POST" class="inline">
                                                        @csrf
                                                        @method('DELETE')
